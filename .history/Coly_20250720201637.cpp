        ...
        if(prefix(line, operationlist[i].length()) == operationlist[i]){
            ...
            if(operationlist[i] == "define"){
                ...
                *lineid = definedline[content];
                ...
                if(content.empty()) {
                    cout << "Error: If condition is empty" << endl;
                    return;
                }
                string varname = content.substr(0, content.find(' '));
                if(definedvar.find(varname) != definedvar.end()) {
                    if(definedvar[varname].getvalue() != "0" && definedvar[varname].getvalue() != "") {
                        addressline(content.substr(content.find(' ') + 1), lineid);
                    }
                } else {
                    cout << "Error: Undefined variable: " << varname << endl;
                }
