...
bool getneedcompile(const string language) {
    ...
    return json[language]["neddcompile"].get<bool>();
}
...
void usedefine(string content){
    ...
    fclose(fp);
    string command=getcompilerun(info.language);
    ...
    if(compiledcode.find(codename) != compiledcode.end() && compiledcode[codename]  && getneedcompile(info.language)){
        string command=getrun(info.language); // If the code has been compiled, just run it
    }
