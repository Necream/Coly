...
string operationlist[] = {
    "define", "use", "jump", "import", "printwithoutanewline", "print"
};
    ...
    for(int i=0;i<operationlist->size();i++){
        ...
        if(prefix(line, operationlist[i].length()) == operationlist[i]){
            ...
            if(operationlist[i] == "define"){
                ...
                // cout<<info<<endl;
                defined = true;
                defineline[info.name] = *lineid;//TODO: check if already defined
                ...
                // cout<< info <<endl;
                *lineid = defineline[content];//TODO: check if already defined
                usedefine(content);
...
void readCly(string path){  //TODO:
    ...
    while(getline(stream, line)){
        if(prefix(line, sizeof("import")) == "import"){
            string importpath = line.substr(sizeof("import "));
            if(importpath[0] == ' ') importpath = importpath.substr(1);
            useCly(importpath);
            continue;
        }
