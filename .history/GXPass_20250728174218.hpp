...
namespace GXPass {
    ...
    type ksm(type a, type n, type mod = std::numeric_limits<type>::max()) {
        if (n == 0) return 1 % mod;
        if (n == 1) return a % mod;

        type temp = ksm<type>(a, n / 2, mod);
        temp = (temp * temp) % mod;
    ...
    template<class type = unsigned long long>
    std::string ksmsum(const std::string& password, type mod = std::numeric_limits<type>::max()) {
        type sum = 0;
        for (char c : password) {
            // 显式指定模板参数，确保推导正确
            sum = (sum + ksm<type>(static_cast<type>(c), static_cast<type>(password.size()), mod)) % mod;
        }
        return c12c2<type, std::string>(sum);
    }
