...
void usedefine(string content){
    ...
    string extension = info.language;
    ...
    if (info.type == "code") {
        fprintf(fp, "%s", info.codeinfo.c_str());
        fclose(fp);
        cout << "Code " << content << " written to " << filename << endl;
        // Execute the code if needed
        GXPass::execute(filename);
    } else if (info.type == "var") {
        fprintf(fp, "%s", info.content.c_str());
        fclose(fp);
        cout << "Variable " << content << " written to " << filename << endl;
    } else {
        cout << "Error: Unknown type for define: " << info.type << endl;
    }
}
