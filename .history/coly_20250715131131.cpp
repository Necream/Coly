...
vector<string> operationlist = {
    "define", "use", "jump", "printwithoutanewline", "print", "do"
};
        ...
        if(prefix(line, operationlist[i].length()) == operationlist[i]){
            ...
            if(operationlist[i] == "define"){
                ...
                *lineid = defineline[content];
                ...
                // cout << "Executing code: " << content << endl;
                defineinfo doinfo = judgedefine(content);
                if(doinfo.type == "code") {
                    usedefine(doinfo.name);
                } else if(doinfo.type == "var") {
                    definedvar[doinfo.name] = doinfo;
                } else {
                    cout << "Error: Invalid type in do command." << endl;
                }
            } else
            {
                /* code */
