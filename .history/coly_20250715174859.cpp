...
map<string, bool> compiledcode; // for checking if a code has been compiled
...
void usedefine(string content){
    ...
    if(compiledcode.find(codename) != compiledcode.end() && compiledcode[codename]){
        cout << "Warning: Code " << codename << " has already been compiled." << endl;
        return; // Skip compilation if already done
    }
