    ...
    for(int i=0;i<content.size();i++){
        ...
        if(c == '$'){
            ...
            if(definedvar.find(varname) != definedvar.end()){
                cout << definedvar[varname].content;
            } else {
...
void addressline(string line,int *lineid){
    ...
    for(int i=0;i<operationlist->size();i++){
        ...
        if(prefix(line, operationlist[i].length()) == operationlist[i]){
            ...
            if(operationlist[i] == "$"){
                // cout << "Variable: " << content << endl;
                if(definedvar.find(content) != definedvar.end()){
                    cout << definedvar[content].content;
                } else {
                    cout << "Error: Undefined variable: " << content << endl;
                }
                return;
            }else if(operationlist[i] == "define"){
