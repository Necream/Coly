...
vector<string> insertsynccode(vector<string> lines, const vector<var> globalvariables) {
    vector<string> newlines;
    newlines.push_back("#include <iostream>");
    newlines.push_back("#include <string>");
    newlines.push_back("#include <vector>");
    newlines.push_back("using namespace std;");
    
    // Insert global variables
    for (const var& v : globalvariables) {
        if (!v.type.empty() && !v.name.empty()) {
            newlines.push_back(v.type + " " + v.name + ";");
        }
    }
    
    // Insert the main function
    newlines.push_back("int main() {");
