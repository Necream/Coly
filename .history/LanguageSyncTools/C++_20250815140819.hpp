...
vector<string> skipliterals(vector<string> lines){
    vector<string> newlines;
    ...
    for(string line:lines){
        ...
        for(int i=0;i<line.size();i++){
            if(line[i]=='"' && !inchar){
                instring=!instring; // toggle string literal
                newstring+=line[i];
            }else if(line[i]=='\'' && !instring){
                inchar=!inchar; // toggle character literal
                newstring+=line[i];
            }else if(instring || inchar){
                newstring+=line[i]; // keep inside literals
            }else{
                if(line[i]!=' ' && line[i]!='\t') newstring+=line[i]; // skip spaces and tabs outside literals
            }
        }
        if(!newstring.empty()) newlines.push_back(newstring);
    }
...
vector<var> getglobalvariables(vector<string> lines){
    lines=skipcomments(lines);
    lines=skipliterals(lines);
    int layer=0;
